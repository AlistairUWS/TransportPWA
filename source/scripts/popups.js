...
$(document).bind('pageinit', function() {
    ...
    if($("#__popups__").length == 0) {
        var __blur = "<style>.__blur{filter:blur(3px);-webkit-filter:blur(3px)}</style>",
            ...
            __popuplist = "<div data-role='popup' id='__jqm_popupList' data-theme='a' class='ui-content' data-position-to='window'><h3 id='__jqm_popup_listHead'>List</h3>" +
                "<ul id='__jqm_popup_list' data-role='listview' data-inset='true'></ul><hidden id='__jqm_popuplist_selected' style='display:none;'></hidden>";


        /*  This code may turn into an extensible pop-up window once the 'trigger'/'create' functionality has been
            figured out.
        // Set up the extendable pop-up window...
        __popupwindow = "<div data-role='popup' id='__jqm_popupWindow' data-theme='a' class='ui-content' data-position-to='window'>" +
                "<form>$$CONTENT$$</form></div>";
        */
        // Append these to the current <body> element...
        $('body').append("<div id='__popups__'>" + __blur + __popupalert + __popupconfirm + __popupprompt + __popuplogin + __popuplist + "</div>")
            .trigger("create");
    }
});
...
function popupAlert(head, text) {
    setTimeout(function () {
        ...
        addBlur();
        $("#__jqm_popupAlert").popup("open").on("popupafterclose", function() {
...
function popupConfirm(head, text, yesFunction, noFunction) {
    setTimeout(function () {
        ...
        //$('#overlay-back').fadeIn(500);
        addBlur();
        $("#__jqm_popupConfirm").popup("open").on("popupafterclose", function() {
            //$('#overlay-back').fadeOut(500);
            removeBlur();
        });
    }, 10);
...
function popupPrompt(head, prompt, okFunction, cancelFunction) {
    setTimeout(function() {
        ...
        //$('#overlay-back').fadeIn(500);
        addBlur();
        $("#__jqm_popupPrompt").popup("open").on("popupafterclose", function() {
            //$('#overlay-back').fadeOut(500);
            removeBlur();
        });
...
function popupLogin(okFunction, cancelFunction) {
    setTimeout(function() {
        ...
        //$('#overlay-back').fadeIn(500);
        addBlur();
        $("#popupLogin").popup("open").on("popupafterclose", function() {
            //$('#overlay-back').fadeOut(500);
            removeBlur();
        });
        $("#userid").focus();
...
function popupList(listHeading, listItems, actionFunction) {
    setTimeout(function() {
        $("#__jqm_popup_listHead").text(listHeading);
        $("#__jqm_popup_list").html(listItems)
            .listview('refresh')
            .on('click', 'li', function() {
                $("#__jqm_popupList").popup("close");
                $("#__jqm_popuplist_selected").text($(this).text());
            });
        addBlur();
        $("#__jqm_popupList").popup("open").on("popupafterclose", function() {
            removeBlur();
            if(actionFunction) {
                actionFunction();
            }
...
function getListSelection() {
    return $("#__jqm_popuplist_selected").text();
}
